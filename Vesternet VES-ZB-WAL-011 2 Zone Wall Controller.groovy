...
metadata {
	definition (name: "Vesternet VES-ZB-WAL-011 2 Zone Wall Controller", namespace: "Vesternet", author: "Vesternet", mcdSync:true, ocfDeviceType: "x.com.st.d.remotecontroller", mnmn: "Sunricher", vid: "generic-4-button") {
        capability "Button"
        capability "Sensor"
		capability "Battery"
...
def addChildButtons(numberOfButtons) {
	...
	for (def endpoint : 1..numberOfButtons) {
		try {
			...
			def child = addChildDevice("Vesternet", "Vesternet VES-ZB-WAL-011 2 Zone Wall Controller Child Button", childDni, device.getHub().getId(), [
					completedSetup: true,
