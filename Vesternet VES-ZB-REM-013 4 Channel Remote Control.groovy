...
metadata {
	definition (name: "Vesternet VES-ZB-REM-013 4 Channel Remote Control", namespace: "Vesternet", author: "Vesternet", mcdSync:true, ocfDeviceType: "x.com.st.d.remotecontroller", mnmn: "Sunricher", vid: "generic-12-button") {
        capability "Button"
        capability "Sensor"
		capability "Battery"
...
def getEvents(descriptionMap) {
    ...
    if (descriptionMap.cluster == "0006" || descriptionMap.clusterId == "0006" || descriptionMap.clusterInt == 6) {
        ...
        if (endpoint == 1) {
            def sceneCommand = "${descriptionMap.command} unknwon"
            switch (descriptionMap.command) {
                case "05": 
...
def addChildButtons(numberOfButtons) {
	...
	for (def endpoint : 1..numberOfButtons) {
		try {
			...
			def child = addChildDevice("Vesternet", "Vesternet VES-ZB-REM-013 4 Channel Remote Control Child Button", childDni, device.getHub().getId(), [
					completedSetup: true,
